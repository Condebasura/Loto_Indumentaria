      ...
      const CheckDatos = (e) => {
         ...
         if (e.target) {
            ...
            return fetch(`http://localhost:3004/Usuarios`)
               .then(res => res.json()
                  .then((data) => {
                     let usuario = data.find(el => InputUser.value === el.InputRegisUser)
                     if (usuario) {
                       

                        if (InputPass.value !== usuario.InputRegisPass) {
                           InputPass.setCustomValidity("La contraseña ingresada es incorrecta");
                        InputPass.reportValidity();
                        } else if(InputPass.value === usuario.InputRegisPass ){
                           InputPass.setCustomValidity("");
                          // InputPass.reportValidity();
                                                }
                     } else {
                        InputUser.setCustomValidity(`El usuario ${InputUser.value} no existe`);
                        InputUser.reportValidity();
                     };
